#!/usr/bin/env python3
"""Build or validate an installable Clock Advanced release archive."""
...
CANONICAL_RESOURCE = "/clock_advanced/clock-advanced-card.js"
LEGACY_RESOURCE = COMPONENT / "frontend" / "clock-advanced.js"
REQUIRED = (
    COMPONENT / "manifest.json",
    COMPONENT / "__init__.py",
    COMPONENT / "config_flow.py",
    COMPONENT / "diagnostics.py",
    COMPONENT / "frontend" / "clock-advanced-card.js",
...
def validate(expected_tag: str | None = None) -> str:
    ...
    if manifest.get("domain") != "clock_advanced":
        raise RuntimeError("manifest domain must be clock_advanced")
    ...
    if 'import "./clock-advanced-card.js"' not in LEGACY_RESOURCE.read_text(encoding="utf-8"):
        raise RuntimeError("legacy resource must import the canonical Card")
...
def build(output: Path, expected_tag: str | None = None) -> None:
    ...
    print(f"Built {output} for Clock Advanced {version}")
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "dist" / "clock_advanced.zip")
    args = parser.parse_args()
    try:
        if args.check:
            print(f"Clock Advanced {validate(args.tag)}: package structure is valid")
        else:
